...
u8 BT_RX_BUF[BT_REC_LEN];     //接收缓冲,最大BT_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
u16 BT_RX_STA=0;       //接收状态标记	
/**********************************
***********************************
BT初始化   作者：陈开放
***********************************
...
void BT_Init(u32 BT_BaudRate)
{
  ...
  //USART 初始化设置
	USART_InitStructure.USART_BaudRate = BT_BaudRate;//串口波特率
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if((BT_RX_STA&0x8000)==0)//接收未完成
		{
			if(BT_RX_STA&0x4000)//接收到了0x0d
			{
				if(Res==0x0a||Res=='n')
					BT_RX_STA|=0x8000;	//接收完成了	
				else
          BT_RX_STA=0;//接收错误,重新开始				
			}
			else //还没收到0X0D
			{	
				if(Res==0x0d||Res=='/')
